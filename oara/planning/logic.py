...
class Condition(Expression):
    ...
    def evaluate(self, trues):
        if self.__condition.evaluate(trues):
            return self.__expression.evaluate(trues)
        else:
            return False

    def simplify(self, trues, falses):
        cond = self.__condition.simplify(trues, falses)
        ...
        if isinstance(cond, TrueExpr):
            ...
            if isinstance(expr, FalseExpr):
                return FalseExpr()
        elif isinstance(cond, FalseExpr):
            return FalseExpr()
        return self
    ...
    def __str__(self):
        return f"({self.__condition} => {self.__expression})"
